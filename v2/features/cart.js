...
function productMaintenanceMessage(product){
  return String(product?.mantenimiento_mensaje||'Temporalmente no disponible. Estamos terminando de habilitar las compras de este juego.');
}
...
async function checkout(method){
  ...
  const maintenanceItem=cart.map(i=>currentProduct(i.product_id)).find(p=>p?.mantenimiento===true);
  if(maintenanceItem){showNotice($('checkoutResult'),productMaintenanceMessage(maintenanceItem),'error');return}
