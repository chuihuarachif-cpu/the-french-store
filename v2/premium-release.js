(() => {
  ...
  const VERSION = 'premium-release-r205-20260914';
  const RELEASE = '20260914-r205-reference-match';
  ...
  function ensureReferenceAssets() {
    ...
    let finalReference = document.getElementById('fs-premium-reference-r205-css');
    if (!finalReference) {
      finalReference = document.createElement('link');
      finalReference.id = 'fs-premium-reference-r205-css';
      finalReference.rel = 'stylesheet';
      finalReference.href = `./tiers/tier-reference-r205.css?v=${RELEASE}`;
      document.head.appendChild(finalReference);
    } else {
      const href = `./tiers/tier-reference-r205.css?v=${RELEASE}`;
    ...
    if (!document.getElementById('fs-premium-reference-r205-js')) {
      const exactArt = document.createElement('script');
      exactArt.id = 'fs-premium-reference-r205-js';
      exactArt.src = `./tiers/tier-reference-r205.js?v=${RELEASE}`;
      exactArt.defer = true;
