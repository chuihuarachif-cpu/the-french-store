(() => {
  ...
  const VERSION = 'premium-release-r202-20260914';
  const RELEASE = '20260914-r202-reference-match';
  ...
  function ensureReferenceAssets() {
    ...
    let finalReference = document.getElementById('fs-premium-reference-r202-css');
    if (!finalReference) {
      finalReference = document.createElement('link');
      finalReference.id = 'fs-premium-reference-r202-css';
      finalReference.rel = 'stylesheet';
      finalReference.href = `./tiers/tier-reference-r202.css?v=${RELEASE}`;
      document.head.appendChild(finalReference);
    } else {
      const href = `./tiers/tier-reference-r202.css?v=${RELEASE}`;
