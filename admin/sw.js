...
const CACHE='fs-admin-r225-install-20260918';
const SHELL=['/admin/','/admin/index.html','/admin/manifest.webmanifest','/v2/assets/brand/icon-192.png','/v2/assets/brand/icon-512.png'];
const STATIC=new Set(['app.css','app.js','r89-auth-fix.js','r93-maintenance-admin.css','r142-maintenance-main-grid.css','r142-maintenance-main-grid.js','r139-quotations.css','r139-quotations.js','r136-granular-maintenance.js','r137-price-overrides.js','r138-account-cost-editor.js','r154-accessibility.css','r154-dialog-focus.js','r164-resellers.js','r165-streaming-fixed-prices.js','r167-seasonal-events.css','r167-seasonal-events.js','r168-admin-tools.css','r168-delivery-generator.js','r169-streaming-reminders.css','r169-streaming-reminders.js','maintenance.html','maintenance.css','maintenance.js','manifest.webmanifest','pwa-install.js'].map(name=>'/admin/'+name));
self.addEventListener('install',event=>{event.waitUntil((async()=>{const cache=await caches.open(CACHE);await Promise.allSettled(SHELL.map(async path=>{const response=await fetch(path,{cache:'reload'});if(response.ok&&!response.redirected)await cache.put(path,response.clone());}));await self.skipWaiting();})());});
