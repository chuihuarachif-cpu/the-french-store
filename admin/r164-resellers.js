/* THE FRENCH STORE — R166 private reseller control center.
   Admin only adds the login email. Rank, reseller number and lifecycle are automatic.
   Legacy RPC intentionally retired from the UI: admin_app_upsert_reseller. */
(() => {
  'use strict';
  ...
  const resellerNo=value=>`RV-${String(Math.max(0,Number(value)||0)).padStart(4,'0')}`;
  let rows=[];
  let installed=false;
  ...
  function injectStyle(){
    ...
    style.textContent=`
      ...
      .r164-level{font-weight:800;color:#9eefff}.r164-number{font-variant-numeric:tabular-nums;letter-spacing:.06em}
      .r164-progress{margin-top:8px;font-size:.88rem;opacity:.86}
      @media(min-width:900px){.r164-grid{grid-template-columns:minmax(330px,.75fr) minmax(0,1.25fr)}}
  ...
  function ensureUi(){
    ...
    if(!button){
      button=document.createElement('button');button.type='button';button.dataset.tab='resellers';button.textContent='🤝 Revendedores';
      nav.insertBefore(button,document.querySelector('[data-tab="history"]')||null);
    }
    if(!$('resellersPanel')){
      const panel=document.createElement('section');
      panel.id='resellersPanel';panel.className='panel hidden';panel.dataset.panel='resellers';
      panel.innerHTML=`
        <div class="panel-head"><div><span class="eyebrow">REVENDEDORES</span><h2>Programa automático</h2><p>Solo agrega el correo con el que inicia sesión. El número, rango y progreso se administran solos.</p></div><button id="resellerRefresh" class="icon-btn" type="button" aria-label="Actualizar">↻</button></div>
        <div class="r164-private"><b>Rangos automáticos:</b> Bronce → Plata → Oro. Plata se alcanza desde Bs 150 de compras del mes y Oro desde Bs 300. Si pasa un mes completo sin ninguna compra, baja un rango. El cliente nunca ve porcentajes ni tu margen.</div>
        <div class="r164-grid">
          <article class="card">
            <div class="card-top"><div><b>Añadir revendedor</b><small>El correo debe ser exactamente el mismo de su inicio de sesión.</small></div></div>
            <div class="r164-add"><label><span>Correo del revendedor</span><input id="resellerEmail" type="email" autocomplete="off" placeholder="cliente@gmail.com"></label><button id="resellerSave" class="primary" type="button">Añadir</button></div>
          </article>
          <div><div id="resellerMetrics" class="r164-metrics"></div><label class="search r164-search"><span>Buscar</span><input id="resellerSearch" type="search" placeholder="Correo o número" autocomplete="off"></label><div id="resellerList" class="list"></div></div>
        </div>
  ...
  function progressText(r){
    const target=Number(r.next_level_target||0);
    if(!target)return 'Rango máximo alcanzado.';
    const missing=Math.max(0,target-Number(r.month_spend||0));
    return missing>0?`Faltan ${money(missing)} este mes para el siguiente rango.`:'El siguiente rango se aplicará automáticamente.';
  }
  ...
  function render(){
    ...
    const list=rows.filter(r=>!q||String(r.email||'').toLowerCase().includes(q)||resellerNo(r.reseller_number).toLowerCase().includes(q));
    host.innerHTML=list.length?list.map(r=>`
      <article class="card ${r.active?'':'attention'}">
        <div class="card-top"><div><b><span class="r164-number">${esc(resellerNo(r.reseller_number))}</span> · ${esc(r.email)}</b><small><span class="r164-level">${esc(r.rank_name||'Bronce')}</span> · ${r.active?'Activo':'Pausado'} · última compra ${esc(date(r.last_order_at))}</small></div><span class="badge ${r.active?'ok':'warn'}">${esc(r.rank_name||'Bronce').toUpperCase()}</span></div>
        <div class="meta"><span>Este mes: ${esc(money(r.month_spend))}</span><span>Mes anterior: ${esc(money(r.previous_month_spend))}</span><span>Total: ${esc(money(r.lifetime_spend))}</span><span>Pedidos: ${esc(r.order_count)}</span></div>
        <div class="r164-progress">${esc(progressText(r))}</div>
      ...
      </article>`).join(''):'<article class="card"><small>No hay revendedores que coincidan.</small></article>';
  ...
  async function save(){
    const email=String($('resellerEmail')?.value||'').trim().toLowerCase();
    ...
    try{
      ...
      $('resellerEmail').value='';
      notify(`${resellerNo(data?.reseller_number)} añadido en rango ${data?.rank_name||'Bronce'}.`);
      await load();
    }catch(error){
      const messages={ADMIN_APP_FORBIDDEN:'Acceso rechazado.',INVALID_EMAIL:'Escribe un correo válido.'};
      notify(messages[error?.message]||'No se pudo añadir el revendedor.');
    }finally{button.disabled=false;}
  }

  async function toggle(email){
    const r=rows.find(x=>String(x.email).toLowerCase()===String(email).toLowerCase());if(!r)return;
    try{
      await rpc('admin_app_set_reseller_active',{p_email:r.email,p_active:!r.active});
  ...
  function install(){
    ...
    $('resellerSave')?.addEventListener('click',save);$('resellerEmail')?.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();save();}});$('resellerRefresh')?.addEventListener('click',()=>load().catch(()=>{}));$('resellerSearch')?.addEventListener('input',render);
  }
