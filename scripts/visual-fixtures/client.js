...
(() => {
 ...
 function report(){const root=document.documentElement;const evidence={qa:true,rank,view,scenario,width:innerWidth,bootstrap:root.dataset.fsBootstrap,membership:root.dataset.fsMembership||'base',motion:root.dataset.r8Motion,overflow:root.scrollWidth>innerWidth,overflowElements:Array.from(document.querySelectorAll('body *')).filter(e=>e.getClientRects().length&&getComputedStyle(e).position!=='fixed'&&e.getBoundingClientRect().right>innerWidth+1).slice(0,6).map(e=>e.id||e.className),errors:errors.slice(0,8),calls:counters,elapsed:Date.now()-started};let out=document.getElementById('fsQaEvidence');if(!out){out=document.createElement('pre');out.id='fsQaEvidence';out.hidden=true;document.body.append(out);}out.textContent=JSON.stringify(evidence);}
