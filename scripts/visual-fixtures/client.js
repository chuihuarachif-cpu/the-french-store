...
(() => {
 ...
 function report(){const root=document.documentElement;const evidence={qa:true,rank,view,scenario,width:innerWidth,bootstrap:root.dataset.fsBootstrap,membership:root.dataset.fsMembership||'base',motion:root.dataset.r8Motion,heroAnimation:getComputedStyle(document.querySelector('.hero'),'::after').animationName,ribbonAnimation:document.getElementById('fsRankPremiumRibbon')?getComputedStyle(document.getElementById('fsRankPremiumRibbon'),'::after').animationName:'none',overflow:root.scrollWidth>innerWidth,overflowElements:Array.from(document.querySelectorAll('body *')).filter(e=>e.getClientRects().length&&getComputedStyle(e).position!=='fixed'&&e.getBoundingClientRect().right>innerWidth+1).slice(0,6).map(e=>e.id||e.className),errors:errors.slice(0,8),calls:counters,elapsed:Date.now()-started};let out=document.getElementById('fsQaEvidence');if(!out){out=document.createElement('pre');out.id='fsQaEvidence';out.hidden=true;document.body.append(out);}out.textContent=JSON.stringify(evidence);if(parent!==window)parent.postMessage(evidence,location.origin);}
 window.addEventListener('load',async()=>{
  ...
  else if(view==='qr'){document.getElementById('qrOrderCode').textContent='QA · vista sin pedido real';document.getElementById('qrOrderTotal').textContent='Total de ejemplo: Bs 18,28';openModal('qrModal');}
