...
(() => {
 ...
 const counters={},errors=[],started=Date.now();
 ...
 function report(){const root=document.documentElement;const evidence={qa:true,rank,view,scenario,width:innerWidth,bootstrap:root.dataset.fsBootstrap,membership:root.dataset.fsMembership||'base',motion:root.dataset.r8Motion,features:{wallet:root.dataset.fsWalletReady,orders:root.dataset.fsOrdersReady,checkout:root.dataset.fsCheckoutReady},qrDecorator:document.getElementById('qrWhatsapp')?.dataset.bisaReady==='1',heroAnimation:getComputedStyle(document.querySelector('.hero'),'::after').animationName,ribbonAnimation:document.getElementById('fsRankPremiumRibbon')?getComputedStyle(document.getElementById('fsRankPremiumRibbon'),'::after').animationName:'none',overflow:root.scrollWidth>innerWidth,overflowElements:Array.from(document.querySelectorAll('body *')).filter(e=>e.getClientRects().length&&getComputedStyle(e).position!=='fixed'&&e.getBoundingClientRect().right>innerWidth+1).slice(0,6).map(e=>e.id||e.className),errors:errors.slice(0,8),calls:counters,elapsed:Date.now()-started};let out=document.getElementById('fsQaEvidence');if(!out){out=document.createElement('pre');out.id='fsQaEvidence';out.hidden=true;document.body.append(out);}out.textContent=JSON.stringify(evidence);if(parent!==window)parent.postMessage(evidence,location.origin);}
 window.addEventListener('load',async()=>{
  ...
  const feature={wallet:'wallet',pedidos:'orders',cart:'checkout',qr:'checkout'}[view];
  if(feature)await window.FSFeatureLoader.ensure(feature);
  if(view==='cart'){addToCart(904);setQty(904,1);renderCart();openModal('cartModal');}
